...
function AdminShows() {
  ...
  const filtered = q ? items.filter((s) => s.title.toLowerCase().includes(q.toLowerCase())) : items;
        ...
        <table className="w-full text-sm">
          <thead className="bg-card text-left text-xs uppercase text-muted-foreground">
            <tr>
              ...
              <th className="px-4 py-3 hidden sm:table-cell">Seasons</th>
          ...
          <tbody>
            ...
            {!loading && filtered.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-6 text-center text-muted-foreground">No shows.</td></tr>
            )}
            {filtered.map((s) => (
              <tr key={s.id} className="border-t border-border/60">
                <td className="px-4 py-3 font-medium">{s.title}</td>
                <td className="px-4 py-3 hidden sm:table-cell text-muted-foreground">{s.slug}</td>
                <td className="px-4 py-3 hidden sm:table-cell text-muted-foreground">{s.seasons.length}</td>
                <td className="px-4 py-3 text-right">
                  <div className="flex justify-end gap-1">
                    <button
                      onClick={() => copyAccessLink(s.slug, s.id)}
                      className="grid h-8 w-8 place-items-center rounded-full text-primary hover:bg-primary/10"
                      title="Copy temporary access link"
                    >
                      {copyingId === s.id ? <Check className="h-3.5 w-3.5" /> : <LinkIcon className="h-3.5 w-3.5" />}
                    </button>
                    <Link
                      to="/admin/shows/$id"
                      params={{ id: s.id }}
